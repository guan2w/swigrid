...
1. 关键可交互元素必须提供稳定 `accessibilityIdentifier`（命名建议：`screen.area.action`）。
2. UI 测试选择器必须与真实控件类型一致（例如 `CollectionView`/`Table` 不能混用）。
3. UI 测试同步优先等待“可交互状态”（`enabled/hittable`），避免仅靠文案值判断时序。
4. 默认关闭 UI Test 并行 worker，避免 clone 模拟器带来的额外资源占用与不稳定性。
